...
1. sample `N` individuals from a binomial distribution of size 1 and $Y \sim Bin(myprob)$
...
For each simulation, kerning (adjusting of values) has been performed after N samples has been drawn from Bernoulli ($Y \sim Bin(myprob)$) distribution. To introduce false positives, a percent of negative cases has been replaced by a vector of equal size where values have been drawn from a Bernoulli distribution with probability set at `fp` ($K_{FP} \sim Bin(fp)$). To introduce false negative, a percentage of positive cases has been replaced by a vector of equal size where values have been drawn from Bernoulli distribution with probability $K_{FN} \sim Bin(1 - fn)$.

```{r preapre_parameteres}
...
```{r figure_1_2, out.width = "\\linewidth"}
plotPower(sims_1_2)
```
...
```{r figure_1_2_2, out.width = "\\linewidth"}
plotCI(sims_1_2)
```
...
```{r figure_3_4_2, out.width = "\\linewidth"}
plotCI(sims_3_4)
```
...
sims_3_4$source <- "intervention"

sims <- do.call(rbind, list(sims_1_2, sims_3_4))
...
Processing took `r round(simtime, 3)` `r attr(simtime, "units")`.